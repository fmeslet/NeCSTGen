...
def gen_pcap(model,
    ...
    num_df_feat = len(columns) # Add flow part information
    ...
    df_ts_tmp = write_to_pcap(model=model,
                          ...
                          dict_bandwidth=dict_bandwidth,
                          dict_spreading_factor=dict_spreading_factor,
                          dict_code_rate=dict_code_rate,
...
def write_to_pcap(model,
    ...
    feat_dim_bandwidth = len(
        dict_bandwidth.values())
    feat_dim_spreading_factor = len(
        dict_spreading_factor.values())
    feat_dim_code_rate = len(
        dict_code_rate.values())
    feat_dim_frequency = len(
        dict_frequency.values())
    ...
    spreading_factor_all_lstm = []
    spreading_factor_all_vae = []
    ...
    code_rate_all_lstm.extend([0]*timesteps)
    code_rate_all_vae.extend([0]*timesteps)

    bandwidth_all_lstm.extend([0]*timesteps)
    bandwidth_all_vae.extend([0]*timesteps)
    
    spreading_factor_all_lstm.extend([0]*timesteps)
    spreading_factor_all_vae.extend([0]*timesteps)
    
    frequency_all_lstm.extend([0]*timesteps)
    frequency_all_vae.extend([0]*timesteps)
    
    crc_status_all_lstm.extend([0]*timesteps)
    crc_status_all_vae.extend([0]*timesteps)
    
    length_all_lstm.extend([0]*timesteps)
    length_all_vae.extend([0]*timesteps)
    
    snr_all_lstm.extend([0]*timesteps)
    snr_all_vae.extend([0]*timesteps)
    
    rssi_all_lstm.extend([0]*timesteps)
    rssi_all_vae.extend([0]*timesteps)
    
    header_length_all_lstm.extend([0]*timesteps)
    header_length_all_vae.extend([0]*timesteps)
    
    fcnt_all_lstm.extend([0]*timesteps)
    fcnt_all_vae.extend([0]*timesteps)
    ...
    for i in range(1, num_packets): # First packet is trash !
        ...
        code_rate_raw_vae = df_feat[i-1][2]
        bandwidth_raw_vae = df_feat[i-1][4]
        spreading_factor_raw_vae = df_feat[i-1][5]
        frequency_raw_vae = df_feat[i-1][6]
        crc_status_raw_vae = df_feat[i-1][7]
        length_raw_vae = df_feat[i-1][9]
        time_diff_raw_vae = df_feat[i-1][10]
        snr_raw_vae = df_feat[i-1][11]
        rssi_raw_vae = df_feat[i-1][12]
        header_length_raw_vae = df_feat[i-1][13]
        payload_length_raw_vae = df_feat[i-1][14]
        fcnt_raw_vae = df_feat[i-1][15]
        
        #['bandwidth', 'spreading_factor', 'frequency', 'crc_status'
        #  'code_rate', 'time_diff', 'snr', 'rssi', 'length_total',
        #  'header_length']
        
        #x = tf.concat(
        #    [x_bandwidth, x_spreading_factor,
        #     x_code_rate, x_feat], axis=-1)
        
        start_idx = 0
        end_idx = feat_dim_bandwidth
        bandwidth_raw_lstm = pred_raw[
            :, start_idx:end_idx][0]
        
        start_idx = feat_dim_bandwidth
        end_idx = feat_dim_bandwidth+feat_dim_spreading_factor
        spreading_factor_raw_lstm = pred_raw[
            :, start_idx:end_idx][0]
        
        start_idx = feat_dim_bandwidth+feat_dim_spreading_factor
        end_idx = feat_dim_bandwidth+feat_dim_spreading_factor+feat_dim_code_rate
        code_rate_raw_lstm = pred_raw[
            :, start_idx:end_idx][0]
        
        start_idx = feat_dim_bandwidth+feat_dim_spreading_factor+feat_dim_code_rate
        end_idx = feat_dim_bandwidth+feat_dim_spreading_factor+feat_dim_code_rate+feat_dim_frequency
        frequency_raw_lstm = pred_raw[
            :, start_idx:end_idx][0]
        
        crc_status_raw_lstm = pred_raw[:, end_idx][0]
        time_diff_raw_lstm = pred_raw[:, end_idx+1][0]
        snr_raw_lstm = pred_raw[:, end_idx+2][0]
        rssi_raw_lstm = pred_raw[:, end_idx+3][0]
        length_raw_lstm = pred_raw[:, end_idx+4][0]
        header_length_raw_lstm = pred_raw[:, end_idx+5][0]
            
        # LOG    
        #['time_diff', 'rate', 'rolling_rate_byte_sec',
        #  'rolling_rate_byte_min', 'rolling_rate_packet_sec', 
        #  'rolling_rate_packet_min', 'fcnt']
        
        #raw_vae_log = [time_diff_raw_vae]
        #raw_vae_log_list = [time_diff_all_vae]
        #raw_vae_log_col = ["time_diff"]

        raw_vae = [code_rate_raw_vae, bandwidth_raw_vae, spreading_factor_raw_vae, 
                      frequency_raw_vae, crc_status_raw_vae, length_raw_vae,
                      snr_raw_vae, rssi_raw_vae, header_length_raw_vae, payload_length_raw_vae]
        raw_vae_list = [code_rate_all_vae, bandwidth_all_vae, spreading_factor_all_vae, 
                          frequency_all_vae, crc_status_all_vae, length_all_vae,
                          snr_all_vae, rssi_all_vae, header_length_all_vae, payload_length_all_vae]
        raw_vae_col = ["code_rate", "bandwidth", "spreading_factor", 
                       "frequency", "crc_status", "length_total",
                       "snr", "rssi", "header_length", 'payload_length']
        
        #raw_lstm_log = [time_diff_raw_lstm]
        #raw_lstm_log_list = [time_diff_all_lstm]
        #raw_lstm_log_col = ["time_diff"]
        
        cat_raw_lstm = [bandwidth_raw_lstm, spreading_factor_raw_lstm, 
                        code_rate_raw_lstm, frequency_raw_lstm]
        cat_raw_lstm_list = [bandwidth_all_lstm, spreading_factor_all_lstm, 
                             code_rate_all_lstm, frequency_all_lstm]
        cat_raw_lstm_dict = [dict_bandwidth, dict_spreading_factor,
                              dict_code_rate, dict_frequency]
        cat_raw_lstm_col = ["bandwidth", "spreading_factor", 
                            "code_rate", "frequency"]
        
        cont_raw_lstm = [crc_status_raw_lstm, #code_rate_raw_lstm,
                       snr_raw_lstm, rssi_raw_lstm, length_raw_lstm, 
                       header_length_raw_lstm]
        cont_raw_lstm_list = [crc_status_all_lstm, #code_rate_all_lstm,
                        snr_all_lstm, rssi_all_lstm, length_all_lstm, 
                        header_length_all_lstm]
        cont_raw_lstm_col = ["crc_status", "snr", "rssi", 
                             "length_total", "header_length"]
        
        
        # SAVE TIME DIFF
        time_diff_lstm = standardize(
                       time_diff_raw_lstm, min_x=0, #df_feat[i, -2], min_x=0, #+timesteps au lieu de 8
        ...
        dict_all_list['timesteps_lstm'] = timesteps_all_lstm
        dict_all_list['time_diff_lstm'] = timesteps_diff_all_lstm
        
        
        time_diff_vae = standardize(
                       time_diff_raw_vae, min_x=0, #df_feat[i, -2], min_x=0, #+timesteps au lieu de 8
                       max_x=1, a=df_raw['time_diff'].min(), # [condition_ports] ?
                       b=df_raw['time_diff'].max())
        ...
        dict_all_list['timesteps_vae'] = timesteps_all_vae
        dict_all_list['time_diff_vae'] = timesteps_diff_all_vae
            
        # LSTM
    
        for f, l, c, d in zip(cat_raw_lstm, 
                              cat_raw_lstm_list, 
                              cat_raw_lstm_col,
                              cat_raw_lstm_dict):
            #print("")
            #print("[DEBUG] c : ", c)
            #print("[DEBUG] f : ", f)
            feat = np.argmax(f, axis=-1)
            #print("[DEBUG] feat : ", feat)
            feat = d[feat]
            #print("[DEBUG] d : ", d)
            l.append(feat)
            #print("[DEBUG] l : ", l)
            dict_all_list[c+"_lstm"] = l  
            
        #print("")
        #print("[DEBUG] dict_all_list : ", dict_all_list)
        
        for f, l, c in zip(cont_raw_lstm, cont_raw_lstm_list, cont_raw_lstm_col):
            feat = standardize( # A CHANGER !! C'est SALE !
                       f, min_x=0, # +timesteps
                       max_x=1, a=df_raw[c].astype(float).min(),
                       b=df_raw[c].astype(float).max())
            l.append(feat)
            dict_all_list[c+"_lstm"] = l
            
        #print("")
        #print("[DEBUG] dict_all_list AFTER cont_raw_lstm : ", dict_all_list)
            
        #print("JE PASSE")
            
        # VAE
        
        for f, l, c in zip(raw_vae, raw_vae_list, raw_vae_col):
            feat = standardize( # A CHANGER !! C'est SALE !
                       f, min_x=0, # +timesteps
                       max_x=1, a=df_raw[c].astype(float).min(),
                       b=df_raw[c].astype(float).max())
            l.append(feat)
            dict_all_list[c+"_vae"] = l
          
        #print("")
        #print("[DEBUG] dict_all_list AFTER raw_vae: ", dict_all_list)
...
for c in ['time_diff', 'rate', 'rolling_rate_byte_sec', # 'length_total', rajouté !!!
          'rolling_rate_byte_min', 'rolling_rate_packet_sec', 
          'rolling_rate_packet_min', 'fcnt']:
...
columns_enc = ['fport', 'mtype', 'code_rate', 'size', 'bandwidth',
               'frequency', 'crc_status', 'gateway']
df_raw[columns_enc] = df_raw[columns_enc].astype(str)
columns_add = ['length_total', 'snr', 'rssi', 'fcnt',
                'header_length', 'payload_length','spreading_factor']
df_raw_tmp = df_raw.fillna(0)
#df_raw = df_raw.fillna(0)

processing = Processing(df=df_raw_tmp.copy(), 
                        arr=arr_raw,
                        columns_enc=columns_enc,
                        columns_add=[])

processing.process(normalize=False)
df_raw = processing.df_process.fillna(0)

cols_log = ['time_diff', 'rate', 'rolling_rate_byte_sec', 
           'rolling_rate_byte_min',  'rolling_rate_packet_sec', 
            'rolling_rate_packet_min'] + columns_add
df_raw[cols_log] = df_raw_tmp[cols_log]
...
#X = data[columns].values 
nb_packet_gen = 20_000 # e.q to packet when flo are not present

...
print("[DEBUG] data['time_diff'].values[0:30] : ", data['time_diff'].values[0:30])
print("[DEBUG] df_concat['time_diff_update'].values[0:30] : ", df_concat['time_diff_update'].values[0:30])
...
lstm_vae_raw_feat = Predictor(
     latent_dim=value_dim, feat_dim=4, feat_dim_flags=0)
...
lstm_vae_raw_feat.save(f"{MODELS_DIR}LSTM_{FULL_NAME}_{PROTO}_{PROTO}_SCAPY_FLOW_NO_CONNECT_FINAL", save_format='tf')

        ...
        while(len(sample_label) != length):
            ...
            if (i == 0):
                if (init_start):
                    ...
                    labels_val, labels_count = np.unique(
                        labels, return_counts=True)
                    labels_count = labels_count / labels_count.sum()
                    
                    labels_choice = np.random.choice(
                        labels_val, size=1, p=labels_count)
...
def gen_seq_labels(generator, 
    ...
    for i in range(nb_flow_ref):
        ...
        # Extract random flow id
        #val = np.random.choice(
        #    df_raw["flow_id"].values)
...
for c in [#'length_total', #'time_diff',
          'length_total', 'header_length', 'payload_length',
          'time_diff', 
          'rate', 
...
generator = Generator(vae=vae, limit_predict=LIMIT_PREDICT)
generator.dict_gmm = gmm

lstm = tf.keras.models.load_model(f"{MODELS_DIR}LSTM_{FULL_NAME}_{PROTO}_{PROTO}_SCAPY_FLOW_NO_CONNECT_FINAL", 
                                  custom_objects={"LeakyReLU": tf.keras.layers.LeakyReLU})
...
result = gen_pcap(model=lstm,
                  ...
                  generator=generator,
                  nb_flow_ref=flows_length_gen.size-1,
                  flows_length_gen=flows_length_gen, # len(flows_length_gen) => nombre de flow à generer
                  forward_steps=FORWARD_STEPS,
                  with_flags=WITH_FLAGS, # IF HTTP or other !!!
                  filename=f"{RESULTS_DIR}N{i}_PCAP_{FULL_NAME}_{PROTO}_ONE_FLOW_SCAPY.pcap")


if (FROM_FLOWS_GEN):
    result[1].to_csv(f"{RESULTS_DIR}DF_GEN_PACKET_{PROTO}_FLOW_NO_CONNECT{EXT_NAME}_FROM_FLOWS_GEN.csv", index=False)
else:
    result[1].to_csv(f"{RESULTS_DIR}DF_GEN_PACKET_{PROTO}_FLOW_NO_CONNECT{EXT_NAME}.csv", index=False)

print(f"[DEBUG] filename : {RESULTS_DIR}DF_GEN_PACKET_{PROTO}_FLOW_NO_CONNECT{EXT_NAME}.csv")
